...
- **Phase 4 — Per-beat generation. Code done, unit-verified; live spot-check pending.** Per-beat Claude calls via structured outputs, one few-shot per primitive (all 7) bundled into a single fully-static, cache-optimized system prompt (stronger than originally planned — every beat call across every lesson shares the cache prefix, not just within one lesson). Two pre-render checks (contract compliance, §7 denylist) each with a feedback-guided retry; a separate transient-error classifier (`app/generation/retry.py`) retries genuine API blips (5xx/429/connection errors) without burning the quality-retry budget on a bug. Real mode now returns a fully generated (not yet execution-validated) lesson. *Exit criterion (generated beats render for ~10 topics) not yet met live*: two attempts at the live spot-check (`scripts/spot_check_beats.py`, 5 topics) both hit sustained `529 Overloaded` from Anthropic specifically on beat-generation calls — not a bug (all 10 plan calls across both runs succeeded fine; the retry path itself was exercised and worked as designed, it just landed inside the same overload window twice). ~$0.22 spent, zero wasted on bugs. Deferred until the API's overload clears; rerun `scripts/spot_check_beats.py` to close this out. Offline verification is solid: 34 tests pass (covering both retry paths, transient-error retry, contract/denylist retries), ruff + mypy clean.
