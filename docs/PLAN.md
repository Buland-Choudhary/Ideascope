...
- **Phase 10 — Deployment & demo readiness. 🟡 STARTED EARLY.** Decided to deploy incrementally rather than wait for full feature completeness, so progress is checkable from any device after each change. `backend/Dockerfile` was fixed along the way (was a stale Phase-0 stub missing `anthropic`/`slowapi`/`playwright` deps entirely — would not have booted) and rebuilt on `uv sync`, verified locally (561MB image, boots clean, `/api/health` + `POST /api/lessons` both work in a container). `render.yaml` (backend) and `frontend/netlify.toml` (frontend) target Render + Netlify's free tiers, chosen after comparing current terms across Render, Fly.io, Railway, Netlify Functions, Cloud Run, and Oracle Cloud.
  ...
  **Post-deploy iteration round 3 (per-lesson color palette, wider model picker, mobile).** Three user-requested changes. (1) **Plan-chosen color palette**: every lesson used to render against one fixed, hardcoded indigo/amber palette baked into the beat-generation prompt, regardless of topic. The plan stage now chooses a `Palette` (`background`/`primary`/`secondary`/`text`/`muted`, all hex) to fit the specific topic's mood (`LessonPlan.palette`, required — every real plan call must actively pick one; `Lesson.palette` carries a default-factory fallback to the old house palette so hand-authored fixtures that predate this field keep validating and rendering with their original colors). Followed the same precedent as `ctx.gsap`: rather than threading literal hex values through the beat-generation prompt/LLM output (risking a mistyped color and busting the beat-prompt's cache-friendliness across lessons), `ctx.palette` is a new host-injected scene-runtime contract field (docs/SCENE_CONTRACT.md §2.2) — the model just writes `ctx.palette.primary` etc. once, generically, and the frontend (`sceneRuntime.ts`), the backend's Python-ported validator runtime (`app/validation/runtime.py`), and the render-check pipeline (`render_check.py`/`pipeline.py`, threaded through so a real render-check — when validation is enabled — sees the real lesson palette instead of an undefined `ctx.palette`) all inject it at render time. All 7 few-shot examples in `app/generation/beat_prompts.py` were rewritten from hardcoded hex to `ctx.palette.*` references. (2) **Wider model picker**: added Claude Fable 5 (`claude-fable-5`) and Claude Opus 5 (`claude-opus-5`) to `ALLOWED_GENERATION_MODELS`/`PRICING_PER_MTOK`/the frontend's `MODEL_LABELS` — deliberately *not* every current Anthropic model: Opus 4.7/4.6 and Sonnet 4.6 stay excluded because they're not on the `claude-api` skill's confirmed structured-outputs support list, and every single generation call in this app (plan/beat/code-review/critique) depends on `client.messages.parse()`. The cost-conscious defaults (`skip_validation=true`, `beat_model=claude-sonnet-5`) are unchanged — this only widens the optional per-lesson override, "Auto (server default)" still resolves to the same cheap path. (3) **Mobile-responsive fixes**: the topic form's duration/model/Generate row and the player header's narration/exit/cost-badge row were single non-wrapping flex rows — fine at 3 items when the model picker had short labels, but the wider model picker's longer labels (e.g. "Fable 5 — most capable, priciest") made the `<select>` grow unboundedly and risked overflow on narrow viewports. Both rows now wrap (`flex-wrap`, capped `max-w` on the model select, full-width Generate button on its own line below `sm:`) and the player header stacks vertically below the `sm:` breakpoint with a `break-words` title. *115 backend + 42 frontend tests pass (10 new — a `ctx.palette` contract-prompt assertion, a per-few-shot check that no primitive regressed to a hardcoded color, and two real-headless-Chromium render-check tests confirming a custom palette actually reaches `ctx.palette` and that the old fixture-compatible default still works), ruff/mypy/eslint/tsc all clean.* **Live-verified against the real Anthropic API and real dev servers, not just mocks**: three plan-only calls for deliberately different-mood topics came back with distinctly on-theme palettes (rainforest photosynthesis → dark green/leaf-green/warm-yellow; jet engine → dark blue-gray/amber/sky-blue; deep space → near-black/cool-blue/warm-orange — $0.0566, 252 in / 2,214 out tokens), then one full real lesson ("a coral reef ecosystem", 8 beats, `skip_validation=true`/`claude-sonnet-5` — the deployed defaults) ran end-to-end through the actual API, and its real LLM-chosen palette (`background:#0e2a33` deep teal, `primary:#f4a259` coral orange, `secondary:#4ec9b0` seafoam) rendered correctly through the real sandboxed runtime — screenshot-confirmed, an on-theme coral-polyp scene in exactly those colors ($0.5527, 15,728 in / 35,551 out tokens — this run also hit a pre-existing, unrelated reliability issue where two of the 8 beats occasionally exhaust their 4096-token output budget on complex SVG scenes and degrade to the text fallback; not a regression from this round, not fixed here). The expanded model picker was live-verified via a real `GET /api/models` call (all 5 models, correctly priced, $0 cost — it's a static config endpoint). The mobile layout was live-verified at a real 375×812 viewport in real headless Chromium: no horizontal overflow on the landing form (with the model picker visible), the player header (narration + exit, then narration + exit + would-be cost badge), or a beat with a manipulable slider — screenshot-confirmed. Total real Anthropic spend this round: **~$0.61**.
