...
- **Phase 10 — Deployment & demo readiness. 🟡 STARTED EARLY.** Decided to deploy incrementally rather than wait for full feature completeness, so progress is checkable from any device after each change. Split into two stages: **Stage A (in progress)** — free hosting, mock-mode-only backend (no Chromium needed, confirmed by reading the generation orchestrator: mock mode never touches the validation pipeline). `backend/Dockerfile` was fixed (was a stale Phase-0 stub missing `anthropic`/`slowapi`/`playwright` deps entirely — would not have booted) and rebuilt on `uv sync`, image verified locally (561MB, boots clean, `/api/health` + mock `POST /api/lessons` both work in a container) without installing the Chromium binary. `render.yaml` (backend, Render free tier, Docker runtime) and `frontend/netlify.toml` (frontend) added after comparing current free-tier terms across Render, Fly.io, Railway, Netlify Functions, Cloud Run, and Oracle Cloud — Render/Netlify won on being genuinely free and already familiar. **Stage B (not started)** — once ready for live generation: swap to the Playwright-Chromium base image, move off Render's free 512MB (too small for Chromium; needs the $25/mo 2GB tier or a per-second host like Fly.io), then the rest of this phase (custom domain, demo GIF, §16 checklist). *Exit: §16 checklist fully green — not yet met; accounts/actual deploy still pending.*
