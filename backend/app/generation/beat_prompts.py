...
library, for eased tweens and timelines — see below), `ctx.palette` (both \
engines: this lesson's five-color scheme — `.background`/`.primary`/ \
`.secondary`/`.text`/`.muted`, each a hex string; see the design-system rule \
below), `ctx.width`/`ctx.height` (numbers), `ctx.params` (current manipulable \
...
designed, not just functional. ALWAYS read colors from `ctx.palette` — never \
invent your own hex values or use bare `black`/`#000` or saturated primaries \
like `red`/`blue`; both p5 (`p.background(...)`, `p.fill(...)`, `p.stroke(...)`) \
and raw SVG attributes accept a CSS hex string directly, so e.g. \
`p.fill(ctx.palette.primary)` or `fill: ctx.palette.primary` just works — no \
color math needed. Background: `ctx.palette.background` (canvas: \
`p.background(ctx.palette.background)`; svg: an initial full-size `<rect>` in \
that fill, appended before anything else). Use `ctx.palette.primary` for the \
focal shape/series, `ctx.palette.secondary` for a second series/value that \
needs to read as visually distinct, `ctx.palette.text` for body/label text, \
`ctx.palette.muted` for gridlines/tracks/de-emphasized elements. Give the main \
shape soft depth instead of a flat fill: canvas — set \
`p.drawingContext.shadowColor = ctx.palette.primary` and \
...
block (a fixed, low-alpha `flood-color` like `rgba(15,23,42,0.25)` is fine — \
that's a shadow tint, not part of the lesson palette) and reference it via \
`filter="url(#soft-shadow)"` on the shape. Round rect corners (canvas: \
`p.rect(x, y, w, h, 8)`; svg: `rx="8"`). Give svg text an explicit \
`font-family="system-ui, -apple-system, sans-serif"` and a real weight \
(`font-weight="600"` for labels) — never leave it at the SVG default serif. \
Leave real breathing room: keep the drawn content within roughly the middle \
80% of `ctx.width`/`ctx.height` rather than flush against the edges. \
...
export default function createScene(ctx) {
  ...
  return {
    ...
    draw(p) {
      ...
      p.background(ctx.palette.background);
      p.stroke(ctx.palette.muted); p.strokeWeight(1);
      p.line(w * 0.08, h / 2, w * 0.92, h / 2);
      const k = (2 * Math.PI * 1.5) / w;
      const phase = ctx.reducedMotion ? 0 : p.frameCount * 0.05;
      p.drawingContext.shadowColor = ctx.palette.primary;
      p.drawingContext.shadowBlur = 14;
      p.noFill(); p.stroke(ctx.palette.primary); p.strokeWeight(3);
      p.beginShape();
...
export default function createScene(ctx) {
  ...
  ctx.svg.appendChild(
    make("rect", { x: 0, y: 0, width: ctx.width, height: ctx.height, fill: ctx.palette.background }),
  );
  ...
  for (let i = 0; i < 3; i++) {
    ...
    ctx.svg.appendChild(make("line", { x1: a[0], y1: a[1], x2: b[0], y2: b[1], stroke: ctx.palette.muted, "stroke-width": 2 }));
  }
  const circles = [];
  for (let i = 0; i < 3; i++) {
    const c = make("circle", { cx: pts[i][0], cy: pts[i][1], r, fill: ctx.palette.muted, stroke: ctx.palette.primary, "stroke-width": 2, filter: "url(#soft-shadow)" });
    circles.push(c);
    ctx.svg.appendChild(c);
    const t = make("text", { x: pts[i][0], y: pts[i][1] + 5, "text-anchor": "middle", "font-family": "system-ui, -apple-system, sans-serif", "font-size": 14, "font-weight": 600, fill: ctx.palette.text });
  ...
  const highlight = () => {
    circles.forEach((c, i) => {
      c.setAttribute("fill", i === stage ? ctx.palette.primary : ctx.palette.muted);
...
export default function createScene(ctx) {
  ...
  ctx.svg.appendChild(make("line", { x1: margin, y1: y, x2: ctx.width - margin, y2: y, stroke: ctx.palette.muted, "stroke-width": 2 }));
  const events = ["First", "Second", "Third"];
  events.forEach((label, i) => {
    const x = margin + (i / (events.length - 1)) * (ctx.width - 2 * margin);
    ctx.svg.appendChild(make("circle", { cx: x, cy: y, r: 11, fill: ctx.palette.primary, stroke: ctx.palette.background, "stroke-width": 3 }));
    const t = make("text", { x, y: y - 26, "text-anchor": "middle", "font-family": "system-ui, -apple-system, sans-serif", "font-size": 14, "font-weight": 600, fill: ctx.palette.text });
    t.textContent = label;
    ctx.svg.appendChild(t);
  });
  ctx.ready();
...
export default function createScene(ctx) {
  ...
  return {
    ...
    draw(p) {
      ...
      p.background(ctx.palette.background);
      p.stroke(ctx.palette.muted); p.strokeWeight(1); p.noFill();
      p.circle(cx, cy, r * 2);
      const x = cx + r * p.cos(-angleDeg);
      const y = cy + r * p.sin(-angleDeg);
      p.stroke(ctx.palette.primary); p.strokeWeight(2.5);
      p.line(cx, cy, x, y);
      p.drawingContext.shadowColor = ctx.palette.primary;
      p.drawingContext.shadowBlur = 14;
      p.noStroke(); p.fill(ctx.palette.primary);
      p.circle(x, y, 14);
      p.drawingContext.shadowBlur = 0;
      p.fill(ctx.palette.text);
      p.circle(cx, cy, 6);
      if (firstFrame) { firstFrame = false; ctx.ready(); }
    },
    onParamChange(param, value) {
...
export default function createScene(ctx) {
  ...
  const barA = make("rect", { x: ctx.width * 0.3, width: 64, rx: 8, fill: ctx.palette.primary, filter: "url(#soft-shadow)" });
  const barB = make("rect", { x: ctx.width * 0.58, width: 64, rx: 8, fill: ctx.palette.secondary, filter: "url(#soft-shadow)" });
  ...
  ctx.svg.appendChild(make("line", { x1: ctx.width * 0.15, y1: baseY, x2: ctx.width * 0.85, y2: baseY, stroke: ctx.palette.muted, "stroke-width": 2 }));
  ctx.ready();
  return {
    onParamChange(param, value) {
...
export default function createScene(ctx) {
  ...
  const partA = make("rect", { x: barX, y: barY, height: 40, fill: ctx.palette.primary });
  const partB = make("rect", { y: barY, height: 40, fill: ctx.palette.muted });
...
export default function createScene(ctx) {
  ...
  return {
    ...
    draw(p) {
      ...
      p.stroke(ctx.palette.muted); p.strokeWeight(2);
      ...
      p.drawingContext.shadowBlur = 0;
      p.fill(ctx.palette.text); p.circle(pivotX, pivotY, 7);
      if (firstFrame) { firstFrame = false; ctx.ready(); }
    },
    onParamChange(param, value) {
